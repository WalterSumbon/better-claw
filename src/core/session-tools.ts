...
import { resolveUserId } from './agent-context.js';
import {
  rotateSession,
 ...
 */
export function createSessionTools(fallbackUserId: string) {
...
Returns session ID, SDK session ID, creation time, message count, turns, and cost.`,
  ...
  return { sessionNewTool, sessionListTool, sessionInfoTool };
}
