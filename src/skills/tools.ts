 ...
 */
export function createSkillTools(fallbackUserId: string) {

  /**
   * 获取 skill 索引。
   *
   * 尝试获取用户特定的合并索引（全局 + per-user），
   * fallback 到全局索引。
   */
  function getContextSkillIndex() {
    ...
    return getUserSkillIndex(userId, userDir);
  }
